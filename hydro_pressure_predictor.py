...
model = XGBRegressor()
model.load_model("Hydro_Pressure_Predictor/model/XGBoost.json")
...
on rectngular and cylandrical piers of bridges, here:
...
Do you want to forecast a single sample data or a file?
    ''')
    choice = input('''
Enter 'single' for single sample data or 'file' for a file: 
    ''')
...
Enter the Channel geometry (Rec or Nonrec): ''')
        ...
        file_path = input('''
Input data file should be in CSV format and must include 3 columns as the following:
...
Please input file nme (for example: Hydro_Pressure_Predictor/example_data/data.csv): 
        ''')
